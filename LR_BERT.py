...
def bert_model_scores(train, validate, train_labels, val_labels, config):
    ...
    if config['BERT']['type'] == 'meanpooling':
        model = BertMeanPoolingClassifier(bert_model, device, N_classes=2,
                                          dropout=config['BERT']['dropout'])
    elif config['BERT']['type'] == 'truncated':
        model = BertTruncatedClassifier(bert_model, device, N_classes=2, dropout=config['BERT']['dropout'])
    elif config['BERT']['type'] == 'average':
        model = BertAverageClassifier(bert_model, device, N_classes=2, dropout=config['BERT']['dropout'])
    else:
    ...
    model = finetune_bert(model, train_dataloader, epochs, config, LR= True)
    preds, f1, scores = validate_bert(model, validate, val_labels, LR=True)
...
def LR(args, config):
    ...
    for i, comb in enumerate(combinations):
        ...
        for suspect in range(0, int(len(set(train_df['author_id'])))):
            print(suspect)
    ...
    for j in range(n_authors):
        ...
        print(round(cllrs[j], 3), round(cllrs_min[j], 3), round(cllrs_cal[j], 3))
        if j % 60 == 0:
            with plotting.show() as ax:
                ax.pav(lr_a, truth_a)
            plt.show()
    ...
    output_file = args.output_path + os.sep + 'LR_BERT_' + args.corpus_name + ".csv"
    with open(output_file, 'a', newline='') as file:
        ...
        writer.writerow([round(cllr, 3), round(cllr_min, 3), round(cllr_cal, 3), round(np.mean(cllrs_min), 3), \
                         ...
                         config['BERT']['model'], config['BERT']['type'], config['BERT']['epochs']])
    output_file = args.output_path + os.sep + 'BERTje_LRS_' + args.corpus_name + ".csv"
    with open(output_file, 'a', newline='') as file:
        writer = csv.writer(file)
